const map = new ol.Map({
  target: "map",
  view: new ol.View({
    center: ol.proj.fromLonLat([107.54159416856646, -7.000618601452999]),
    zoom: 12.3,
  }),
});
          ...
          if (feature.properties.curve) {
            ...
            for (let i = 0; i < lineStringCoords.length - 1; i++) {
              ...
              for (let t = 0.1; t <= 0.9; t += 0.1) {
                const x = (1 - t) * (1 - t) * lineStringCoords[i][0] + 2 * (1 - t) * t * curve[i][0] + t * t * lineStringCoords[i + 1][0];

                const y = (1 - t) * (1 - t) * lineStringCoords[i][1] + 2 * (1 - t) * t * curve[i][1] + t * t * lineStringCoords[i + 1][1];
...
addGeoJSONToMapAndTable("https://raw.githubusercontent.com/firdayulianti/gisfirda/main/json/point.json", map, document.querySelector("table"));
addGeoJSONToMapAndTable("https://raw.githubusercontent.com/firdayulianti/gisfirda/main/json/poliline.json", map, document.querySelector("table"));
addGeoJSONToMapAndTable("https://raw.githubusercontent.com/firdayulianti/gisfirda/main/json/polygon.json", map, document.querySelector("table"));
